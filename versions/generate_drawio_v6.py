#!/usr/bin/env python3
"""
SE322 'Register for Courses' — clean B&W draw.io sequence diagram.
Edges use relative=1 so draw.io auto-positions them.
"""
...
GAP   = 140
START = 140
CX = {pid: START + i*GAP for i,pid in enumerate(["S","UI","CT","SV","CR","RR","BS","BL"])}
...
ACTS = [
    ("UI",100,135), ("CT",130,75),
    ("UI",280,215), ("CT",310,155), ("SV",340,95), ("CR",370,30),
    ("UI",530,505), ("CT",560,445), ("SV",590,385), ("CR",620,30),
...
def edge(val, style, src, tgt, relative="1", pts=None):
    cid = str(len(cells)+2)
    vs = f' value="{esc(val)}"' if val else ""
    ss = f' style="{esc(style)}"' if style else ""
    if pts:
        px = "".join(f'              <mxPoint x="{x}" y="{y}" />\n' for x,y in pts)
        cells.append((cid,
            f'        <mxCell id="{cid}"{vs}{ss} edge="1" parent="1" source="{src}" target="{tgt}">\n'
            f'          <mxGeometry relative="{relative}" as="geometry">\n'
            f'            <Array as="points">\n{px}'
            f'            </Array>\n'
            f'          </mxGeometry>\n'
            f'        </mxCell>\n'))
...
# 1. Title
vert(
 "<b>Sequence Diagram: Register for Courses</b><br><i>(N-Tier Architecture — XYZ Tutoring Center — SE322)</i>",
 "text;html=1;strokeColor=none;fillColor=none;align=center;verticalAlign=top;fontFamily=Helvetica;fontSize=14;fontColor=#000000;",
 {"x":"150","y":"2","width":"800","height":"28"})

# 2. Phase labels
for tx,py in [("Authentication",100),("Browse Courses",280),("Register for Courses",530)]:
    vert(f"<b>{tx}</b>",
         "text;html=1;strokeColor=none;fillColor=none;align=left;verticalAlign=top;fontFamily=Helvetica;fontSize=10;fontColor=#888888;",
...
for pid,lab,stereo in PARTS:
    cx = CX[pid]; w = BW[stereo]; x = cx - w//2
    if stereo == "actor":
        lbl = lab
        ...
        sty = ("rounded=1;whiteSpace=wrap;html=1;"
               "fillColor=#FFFFFF;strokeColor=#000000;strokeWidth=1;fontFamily=Helvetica;fontSize=11;")
    cid = vert(lbl, sty, {"x":str(x),"y":"28","width":str(w),"height":"42"})
    box_ids[pid] = cid
...
for pid,lab,stereo in PARTS:
    ...
    cid = vert("",
         ...
         {"x":str(cx-0.5),"y":"76","width":"1","height":"1000"})
    life_ids[pid] = cid
...
for pid,y0,h in ACTS:
    ...
    act_list.append((pid,y0,y0+h,cid))

def tgt(pid,y):
    for apid,a0,a1,aid in act_list:
        if apid==pid and a0<=y<=a1: return aid
    return life_ids.get(pid)

# 6. Messages (edges with relative=1)
F = "fontFamily=Helvetica;fontSize=10;verticalAlign=bottom;align=center;labelBackgroundColor=#FFFFFF;"
CALL = F+"endArrow=blockThin;endFill=1;strokeColor=#000000;strokeWidth=1;exitX=0.5;exitY=0;entryX=0.5;entryY=0;"
RETN = F+"endArrow=open;endFill=0;strokeColor=#666666;dashed=1;strokeWidth=1;exitX=0.5;exitY=0;entryX=0.5;entryY=0;"
SELF = F+"endArrow=blockThin;endFill=1;strokeColor=#000000;strokeWidth=1;entryX=0.5;entryY=0;exitX=0.5;exitY=1;"

for fr,to,lb,y,mt in MSGS:
    if mt=="self":
        src = tgt(fr,y)
        edge(lb, SELF, src, src, relative="1",
             pts=[(CX[fr]+40,y),(CX[fr]+40,y-30)])
    else:
        st = CALL if mt=="call" else RETN
        edge(lb, st, tgt(fr,y), tgt(to,y), relative="1")
...
# ═══ assemble ═══
PAGE_W = 1200; PAGE_H = 1150
L = []
L.append(f'<mxfile host="app.diagrams.net" agent="SE322" version="24.2.5">\n')
L.append(f'  <diagram id="seq-diagram-01" name="Register for Courses">\n')
L.append(f'    <mxGraphModel dx="0" dy="0" grid="1" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" '
         f'fold="1" page="1" pageScale="1" pageWidth="{PAGE_W}" pageHeight="{PAGE_H}" background="#FFFFFF" math="0" shadow="0">\n')
L.append('      <root>\n')
L.append('        <mxCell id="0" />\n')
L.append('        <mxCell id="1" parent="0" />\n')
for _,x in cells: L.append(x)
L.append('      </root>\n')
...
dups  = {k for k,v in __import__('collections').Counter(mxids).items() if v>1}
print(f"✅ XML valid · {len(mxids)} mxCells · {'❌ dup:'+str(dups) if dups else '✅ unique IDs'}")

ret = os.system('drawio --export --format png --output /tmp/test_v6.png "'+OUT+'" 2>/dev/null')
print(f"{'✅' if ret==0 else '❌'} draw.io export {'succeeded' if ret==0 else 'failed'}")

if ret == 0:
    import subprocess
    r = subprocess.run(['sips','-g','pixelWidth','-g','pixelHeight','/tmp/test_v6.png'],
                       capture_output=True, text=True)
    for line in r.stdout.strip().split('\n'):
        if 'pixel' in line: print(f"   {line.strip()}")
